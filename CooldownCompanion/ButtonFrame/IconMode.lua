...
function CooldownCompanion:UpdateButtonIcon(button, totemTexture)
    local buttonData = button.buttonData
    if buttonData.type == "totem" then
        -- Display-only pass-through. Never inspect, compare, or remember the
        -- slot's possibly secret texture, and never resolve a spell from it.
        button.icon:SetTexture(totemTexture)
        return
    end
    local icon
    local hasIcon = false
