...
local function ResolveSpecColorKey(resource, specID, key)
    if specID and type(resource) == "table" then
        local specColors = resource.specColors
        if type(specColors) == "table" then
            local specData = specColors[specID]
...
local function GetResourceColors(powerType, settings)
    ...
    if not def then
        ...
        if settings and settings.resources then
            ...
            if override then
                local resolved = ResolveSpecColorKey(override, specID, "color")
    ...
    local n = #keys
    if n == 2 then
        return ResolveSpecColorKey(override, specID, keys[1]) or defaults[1],
               ResolveSpecColorKey(override, specID, keys[2]) or defaults[2]
    elseif n == 3 then
        return ResolveSpecColorKey(override, specID, keys[1]) or defaults[1],
               ResolveSpecColorKey(override, specID, keys[2]) or defaults[2],
               ResolveSpecColorKey(override, specID, keys[3]) or defaults[3]
    end
...
local function GetSegmentedThresholdConfig(powerType, settings)
    ...
    local specID = GetCurrentSpecID()
    local enabled = ResolveSpecColorKey(resource, specID, "segThresholdEnabled")
    if enabled ~= true then
        return false, nil, nil
    end

    local threshold = tonumber(ResolveSpecColorKey(resource, specID, "segThresholdValue"))
    ...
    local thresholdColor = GetSafeRGBColor(ResolveSpecColorKey(resource, specID, "segThresholdColor"), DEFAULT_SEG_THRESHOLD_COLOR)
...
local function GetContinuousTickConfig(powerType, settings)
    ...
    local specID = GetCurrentSpecID()
    local enabled = ResolveSpecColorKey(resource, specID, "continuousTickEnabled")
    if enabled ~= true then
        return false, nil, nil, nil, nil
    end

    local mode = ResolveSpecColorKey(resource, specID, "continuousTickMode")
    if mode ~= "percent" and mode ~= "absolute" then
        mode = DEFAULT_CONTINUOUS_TICK_MODE
    end

    local percentValue = tonumber(ResolveSpecColorKey(resource, specID, "continuousTickPercent"))
    ...
    local absoluteValue = tonumber(ResolveSpecColorKey(resource, specID, "continuousTickAbsolute"))
    ...
    local tickColor = GetSafeRGBAColor(ResolveSpecColorKey(resource, specID, "continuousTickColor"), DEFAULT_CONTINUOUS_TICK_COLOR)
    local tickWidth = tonumber(ResolveSpecColorKey(resource, specID, "continuousTickWidth")) or DEFAULT_CONTINUOUS_TICK_WIDTH
    if tickWidth < 1 then tickWidth = 1 elseif tickWidth > 10 then tickWidth = 10 end
    local combatOnly = ResolveSpecColorKey(resource, specID, "continuousTickCombatOnly") or false
